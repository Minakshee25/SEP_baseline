#!/bin/bash
# eval_5arm_gpu_swap.sh — RACE-FREE GPU borrow on GPU 1 for the frozen 5-arm same-model
# proxy eval (amortized_ue.eval_samemodel_5arm). Inference only, no training.
# Same pattern as e62_gpu_swap.sh / e63_gpu_swap.sh.
#
#   1. SIGSTOP the GPU1 training-lane script (watchdog only restarts a DEAD pid; a
#      stopped pid still passes kill -0, so the watchdog stays dormant).
#   2. SIGTERM->SIGKILL the Qwen3.6-27B stage1 child -> frees ~41 GB.
#      stage1 is fully resumable: it skips existing record .pt files.
#   3. Fence the freed memory, leaving ~13 GB for the eval.
#   4. Run the eval (hard 40-min cap).
#   5. Drop the fence, SIGCONT the lane -> it reaps the killed child, re-fences,
#      RESUMES Qwen3.6-27B from its last saved record.
set -uo pipefail
...
LOG=amortized_ue/eval_5arm_swap.log

exec >>"$LOG" 2>&1
echo "======================================================================"
echo "[$(date)] 5-arm eval GPU-swap starting (GPU $GPU, interrupting Qwen3.6-27B squad)"
...
echo "[$(date)] launching eval (timeout ${TIMEOUT}s)..."
CUDA_VISIBLE_DEVICES=$GPU timeout $TIMEOUT $PY -m amortized_ue.eval_samemodel_5arm \
  --data_dir /data2/mn1025/stage1
RC=$?
echo "[$(date)] eval exited rc=$RC"

resume
echo "[$(date)] === 5-arm eval GPU-swap complete (rc=$RC) ==="
exit $RC
