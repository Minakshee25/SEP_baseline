#!/bin/bash
# e68_gpu_swap.sh -- RACE-FREE GPU borrow on GPU 1 for the E68 LOLO-squad eval
# (amortized_ue.e68_lolo_squad_llama3_deepseek). Inference only, no training.
...
BUDGET_MIB="${BUDGET_MIB:-16000}"
TIMEOUT="${TIMEOUT:-3000}"
LOG=amortized_ue/e68_gpu_swap.log

exec >>"$LOG" 2>&1
echo "======================================================================"
echo "[$(date)] E68 eval GPU-swap starting (GPU $GPU, interrupting gemma-2-27b-it squad)"
...
echo "[$(date)] launching E68 eval (timeout ${TIMEOUT}s)..."
CUDA_VISIBLE_DEVICES=$GPU timeout $TIMEOUT $PY -m amortized_ue.e68_lolo_squad_llama3_deepseek
RC=$?
echo "[$(date)] eval exited rc=$RC"

resume
echo "[$(date)] === E68 eval GPU-swap complete (rc=$RC) ==="
exit $RC
