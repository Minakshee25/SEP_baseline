#!/bin/bash
# E23 fresh-1000 build (shared held-out ids) WITH GPU memory fencing -- same recipe as
# build_e23_fresh.sh (--num_samples 1000 --selection_num_samples 3074 --only_ids e23_fresh_ids.txt),
# but per attempt it launches a gpu_reserve holder that grabs (free - NEED_MIB - SAFETY) MiB on the
# chosen GPU so a co-tenant OOMs on THAT card instead of landing next to us and starving/OOM-ing our
# run mid-flight (the failure that killed the Llama-3 n2000 build once). Fencing logic mirrors
# build_n2000_waiter.sh; the stage1 invocation mirrors build_e23_fresh.sh. Build is resumable
# (overwrite=False) so each stretch saves records; the holder self-releases if this waiter dies
# (--parent_pid) so it can never orphan-block the GPU.
# Args: MODEL ENV [NEED_MIB]
#   MODEL = Meta-Llama-3-8B-Instruct | Mistral-7B-Instruct-v0.2 | ...
...
MODEL="$1"; ENV="$2"; NEED_MIB="${3:-37000}"; POLL=${POLL:-5}; TARGET=${TARGET:-1000}; SAFETY=${SAFETY:-400}
...
echo ">>> $(date +%H:%M:%S) fenced E23 fresh waiter: model=$MODEL env=$ENV need=${NEED_MIB}MiB target=$TARGET have=$(have)"
while [ "$(have)" -lt "$TARGET" ]; do
  ...
  ( conda activate "$ENV"
    ...
    CUDA_VISIBLE_DEVICES="$PICK" python -m amortized_ue.stage1 \
      ...
      --num_samples "$TARGET" --selection_num_samples 3074 --only_ids scratch_xllm/e23_fresh_ids.txt
    rc=$?
...
echo ">>> $(date +%H:%M:%S) E23 fresh BUILD COMPLETE model=$MODEL have=$(have)/$TARGET"
