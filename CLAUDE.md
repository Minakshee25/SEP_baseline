...
- **`amortized_ue/` (new work — Stages 1 & 2 built; cross-LLM transfer characterised end-to-end. Work has continued well past the bullets below — E43–E73 cover the true LOLO proxy, Qwen/Gemma small- + big-tier transfer, squad OOD, latency/efficiency, the big-tier aligned-ridge, and the big-tier + set-2 per-model supervised ceilings. The bullets here stop at E42; `amortized_ue/CLAUDE.md` "Current state" and `EXPERIMENTS.md` are the live record. Most recent: E73 — E72 applied to the small-tier Qwen/Gemma "set 2": same result (**ridge ≥ proxy ≥ SEP, the frozen 3B proxy never beats a ridge on the same hidden state** — E15–E17 holds at every tier); nuance — E71's cross-model *text* proxy edges the per-model *hidden-state* methods on ID AUROC for small Qwen/Gemma.). See `amortized_ue/CLAUDE.md` (current state + to-do) and `EXPERIMENTS.md` (full history E0→E73).**
