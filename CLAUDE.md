...
- **`amortized_ue/` (new work — Stages 1 & 2 built; cross-LLM transfer characterised end-to-end. Work has continued well past the bullets below — E43–E72 cover the true LOLO proxy, Qwen/Gemma small- + big-tier transfer, squad OOD, latency/efficiency, the big-tier aligned-ridge, and the big-tier per-model supervised ceiling. The bullets here stop at E42; `amortized_ue/CLAUDE.md` "Current state" and `EXPERIMENTS.md` are the live record. Most recent: E72 — big-tier 5×27B PER-MODEL (individual, not LOLO) proxy vs ridge vs SEP on one common layer: **ridge ≥ proxy ≥ SEP everywhere, proxy never beats ridge** (E15–E17 confirmed at 27B & OOD — z→SE is linear); per-model ridge ≈ true 10-sample SE for wrong-answer detection ID; per-model supervised ≫ cross-model transfer.). See `amortized_ue/CLAUDE.md` (current state + to-do) and `EXPERIMENTS.md` (full history E0→E72).**
