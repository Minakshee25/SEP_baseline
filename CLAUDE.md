...
- **`amortized_ue/` (new work — Stages 1 & 2 built; the single-LLM proxy is fully characterised, cross-LLM work is starting). See `amortized_ue/CLAUDE.md` (current state + to-do) and `EXPERIMENTS.md` (full history E0→E20).**
  - **🎯 THE THESIS: CROSS-LLM TRANSFER.** Train the proxy on target-LLM A's data, test whether it predicts SE for a *different* LLM B — motivated by the **Platonic Representation Hypothesis**. This is *why* the proxy is an SLM taking question/response **text** alongside the hidden state (text is model-agnostic). No per-model probe (SEP/ridge) can do this. **Status: cross-LLM experiment #1 DONE (E20)** — frozen Llama-2 proxy → Llama-3-8B on the 200 held-out questions: **hidden states do NOT transfer** (z 0.602→0.056, chance; naive PRH fails for SE) but **text DOES** (q_only 88% retained, q_resp_only full). Only the model-agnostic text pathway survives a model swap → validates the text-reading proxy. Next: a 2nd, non-Llama target to test generality. (Build with `stage1.py --only_ids`; score with `stage2/eval_cross_llm.py`.)
  - **Reference model — SAVED, 25 checkpoints** (`runs/REFERENCE_multipos_p1024_5arm_ckpt/`; TBG L22 + SLT L15, projector 1024, k=4, 5 seeds). **z arm ID Spearman 0.602±0.019 / AUROC 0.807, OOD 0.368**; **q_only 0.494 / 0.758** (SE from question alone). Spearman is primary.
